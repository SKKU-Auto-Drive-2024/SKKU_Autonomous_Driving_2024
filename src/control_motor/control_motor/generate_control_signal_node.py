...
car_center_point_x = 297
car_center_point_y = 447
...
class IntegrateInfo():
  ...
  def process(self, data, decision=0): 
    ...
    diff = float(road_target_point_x - car_center_point_x)
    #좌측으로 벗어난 경우의 보정
    if diff < 70 and diff > 54:
       steering_command = min(7, int(road_gradient/3))
    elif diff > 70:
       steering_command = min(7, int(road_gradient/3))
    
    #우측으로 벗어난 경우의 보정
    elif diff < -56 and diff > -80:
       steering_command = max(-7, int(road_gradient/4))
    ...
    #stable condition
       ...
       if road_gradient < 0:
          steering_command = max(-7, int(road_gradient/3))
       else:
          steering_command = min(7, int(road_gradient/3))
    ...
    print('diff: ', diff)
    print('road_gradient: ', road_gradient)
